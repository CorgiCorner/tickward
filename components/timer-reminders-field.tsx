...
const REMINDER_PRESETS = [5, 10, 30, 60, 1440, 10080] as const
const CUSTOM_UNITS = [
  { value: "minutes", multiplier: 1, labelKey: "timer.form.reminders.unit.minutes" },
  { value: "hours", multiplier: 60, labelKey: "timer.form.reminders.unit.hours" },
  { value: "days", multiplier: 1440, labelKey: "timer.form.reminders.unit.days" },
  { value: "weeks", multiplier: 10080, labelKey: "timer.form.reminders.unit.weeks" },
] as const
...
function messageText(message: InlineMessage) {
  ...
  if (message === "limit") return formatMessage("timer.form.reminders.limit", { max: MAX_TIMER_REMINDERS })
...
function scheduleTargetMs(args: { date?: string; time?: string; timezone?: string }) {
  ...
  try {
    const ms = new Date(wallClockToUtcIso({ date: args.date, time: args.time, timezone: args.timezone })).getTime()
...
export function TimerRemindersField(props: Readonly<{ control: Control<TimerFormValues> }>) {
  const { append, fields, remove } = useFieldArray({ control: props.control, name: "reminders" })
  const reminders = useWatch({ control: props.control, name: "reminders" }) ?? []
  ...
  const repeatEnabled = useWatch({ control: props.control, name: "repeatEnabled" })
  ...
  const now = useNow()
  const targetMs = repeatEnabled ? null : scheduleTargetMs({ date, time, timezone })
      ...
      {reminders.length > 0 ? (
        <div className="flex flex-wrap gap-2">
          {reminders.map((reminder, index) => {
            ...
            return (
              <span
                key={fields[index]?.id ?? `${reminder.offsetMinutes}-${index}`}
                className={cn(
              ...
              >
                ...
                <button
                  ...
                  aria-label={formatMessage("timer.form.reminders.remove", { offset: label })}
                  onClick={() => {
                    remove(index)
                    setMessage(null)
                  }}
