...
describe("prisma project repository", () => {
  ...
  it("imports projects beyond the account limit and reports the overflow as read-only", async () => {
    const { prismaProjectRepository } = await import("./prisma-project-repository.server")
    const prisma = prismaMock()
    const existingCreatedAt = new Date("2026-01-01T00:00:00.000Z")
    prisma.project.findMany.mockResolvedValueOnce([]).mockResolvedValueOnce([
      {
        id: "project_existing",
        ownerId: "user_123",
        claimedAt: existingCreatedAt,
        createdAt: existingCreatedAt,
      },
    ])
    mocks.requirePrismaClient.mockReturnValue(prisma)

    const importedAt = "2026-07-10T18:00:00.000Z"
    const result = await prismaProjectRepository.importUserProjects?.({
      accountPreferences: {
      ...
      projects: [
        {
          ...
          updatedAt: "2026-02-01T00:00:00.000Z",
          claimedAt: null,
          snapshot: makeProjectSnapshot({ name: "First" }),
        },
        {
          id: "project_overflow",
          name: "Overflow",
          ...
          claimedAt: null,
          snapshot: makeProjectSnapshot({ name: "Overflow" }),
        },
      ],
      user: { id: "user_123" },
    })
    ...
    expect(result).toEqual({
      created: ["project_first", "project_overflow"],
      replaced: [],
      conflicts: [],
      accountPreferencesImported: true,
      notificationPreferencesImported: 1,
      profileImported: true,
      readOnlyProjectIds: ["project_overflow"],
    })
    ...
    expect(prisma.userPreference.upsert).toHaveBeenCalledWith({
      ...
      create: expect.objectContaining({ defaultTimezone: "Europe/Warsaw", notificationSound: "chord" }),
      update: expect.objectContaining({ defaultTimezone: "Europe/Warsaw", notificationSound: "chord" }),
    })
    expect(prisma.notificationPreference.upsert).toHaveBeenCalledWith({
      where: {
        userId_targetType_targetId: { userId: "user_123", targetType: "user", targetId: "global" },
      },
      create: expect.objectContaining({
        userId: "user_123",
        targetType: "user",
      ...
      update: expect.objectContaining({ updatedAt: new Date("2026-02-01T00:00:00.000Z") }),
    })
    expect(prisma.project.create).toHaveBeenCalledTimes(2)
    expect(prisma.project.create).toHaveBeenNthCalledWith(
      1,
      expect.objectContaining({
        data: expect.objectContaining({
          id: "project_first",
          ownerId: "user_123",
          claimedAt: new Date(importedAt),
          name: "First",
          color: null,
    ...
    expect(prisma.project.create).toHaveBeenNthCalledWith(
      ...
      expect.objectContaining({
        data: expect.objectContaining({
          id: "project_overflow",
          ownerId: "user_123",
          claimedAt: new Date(new Date(importedAt).getTime() + 1),
        }),
      }),
    )
  })

  it("does not replace a project id owned by another account", async () => {
    const { prismaProjectRepository } = await import("./prisma-project-repository.server")
    const prisma = prismaMock()
    const createdAt = new Date("2026-01-01T00:00:00.000Z")
    prisma.project.findMany
      .mockResolvedValueOnce([{ id: "project_foreign", ownerId: "user_other", claimedAt: null, createdAt }])
      ...
      projects: [
        {
          ...
          claimedAt: null,
          snapshot: makeProjectSnapshot({ name: "Foreign" }),
        },
      ],
      user: { id: "user_123" },
    })
    ...
    expect(result).toEqual({
      created: [],
      replaced: [],
      conflicts: [{ projectId: "project_foreign", reason: "id_unavailable" }],
