...
Use `PATCH /webhooks/{webhook_id}` to edit the subscribed `event_types` or to
...
Use `Send` in Settings to send a one-off `webhook.test` event to the
...
The `Send` menu can also send a sample of any event type the endpoint is
...
After saving the Make webhook, click `Send` in tickward. Make should detect
...
Cloudflare Cron Trigger is the smallest production setup for most self-hosted
deployments. Keep the scheduler secret in Worker secrets and keep the app URL as
a normal Wrangler variable.
...
See `examples/scheduler/cloudflare-worker` for a copy-paste starter.
